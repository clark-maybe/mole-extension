...
本项目是 **MoleClaw** Chrome 扩展程序（MV3），使用 React + TypeScript + Vite 构建。核心功能是**在任意网页上注入一个 AI 悬浮助手（Mole）**，用户通过悬浮球或快捷键唤起对话框，与 AI 交互。AI 通过 Agentic Loop 自主调用 36+ 内置工具，在后台多轮执行，完成后以流式方式返回结果。
