        ...
        void (async () => {
            try {
                ...
                const prompt = [
                    ...
                    '- page_action — 页面元素操作（点击/填写/选择/等待元素，params.action: click/fill/select/wait_for_element）',
                    '- page_snapshot — 获取页面语义快照',
                    '- page_viewer — 获取页面内容',
                    '- js_execute — 执行 JavaScript 代码',
                    '- dom_manipulate — DOM 查询和修改',
                    ...
                    '- 页面导航（URL 变化）使用 tab_navigate，不是 page_action',
                    '- 元素点击/填写/选择使用 page_action',
                    '- 需要执行 JS 时使用 js_execute',
                    '',
                    '输出格式（严格 JSON，无多余文字）：',
                    '{',
                    ...
                    '      { "action": "page_action", "params": { "action": "click", "selector": "..." }, "note": "点击按钮" },',
                    '      { "action": "page_action", "params": { "action": "fill", "selector": "input", "value": "{{keyword}}" }, "note": "输入搜索词" }',
                    '    ]',
                    '  }',
                    '}',
                ].join('\n');
