...
export const bookmarkOpsFunction: FunctionDefinition = {
  name: 'bookmark_ops',
  description: [
    'Manage browser bookmarks. Supported actions:',
    '- add: bookmark current page or a specified URL (optionally into a target folder)',
    '- search: keyword search bookmarks (with pagination)',
    '- recent: list recently added bookmarks',
    '- list: list bookmarks by folder (optionally recursive)',
    '- getAll/get_all: export all bookmarks (with pagination to avoid truncation)',
    '- get: retrieve a single bookmark by ID',
    '- update: update bookmark title/URL',
    '- move: move a bookmark to a target folder',
    '- create_folder: create a bookmark folder',
    '- delete: delete a bookmark (folders are deleted recursively)',
    '- stats: bookmark statistics',
  ].join('\n'),
  ...
  parameters: {
    ...
    properties: {
      ...
      url: { type: 'string', description: 'Bookmark URL (used for add/update)' },
      title: { type: 'string', description: 'Bookmark title (used for add/update/create_folder)' },
      query: { type: 'string', description: 'Search keyword (used for search)' },
      bookmark_id: { type: 'string', description: 'Bookmark ID (used for get/update/move/delete)' },
      folder_id: { type: 'string', description: 'Folder ID (used for add/list/create_folder)' },
      parent_id: { type: 'string', description: 'Parent folder ID for create_folder (compat param)' },
      destination_folder_id: { type: 'string', description: 'Target folder ID for move' },
      index: { type: 'number', description: 'Position index after insert/move (0-based)' },
      include_folders: { type: 'boolean', description: 'Whether to include folder nodes, default false' },
      recursive: { type: 'boolean', description: 'Whether to recursively list subfolder contents for list, default true' },
      sort_by: {
        type: 'string',
        enum: ['date_added', 'title', 'url', 'path'],
        description: 'Sort field, default date_added',
      },
      sort_order: {
        type: 'string',
        enum: ['asc', 'desc'],
      ...
      limit: { type: 'number', description: `Page size, default ${DEFAULT_LIMIT}, max ${MAX_LIMIT}` },
