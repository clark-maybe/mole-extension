...
Mole 不是远程爬虫，也不是模拟浏览器——它直接运行在你正在使用的 Chrome 中。

这带来一个关键优势：**登录态复用**。你在淘宝、GitHub、飞书、企业后台等任何网站上的登录状态，Mole 都可以直接使用。网站看到的就是你本人的访问，不会触发额外的风控。
